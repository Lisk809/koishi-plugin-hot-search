...
export async function handler(ctx: Context, config: Config) {
  ...
  ctx
    ...
    .action(async ({ options }) => {
      ...
      return hotgovs.map(a=>`⏏️  [${a.icon_desc||a.label_name}] ${a.word_scheme||a.word}\n`).join("")+realtime.slice(0, options.num).map(a=>`=${a.rank+1}= [${a.icon_desc||a.label_name}] ${a.word_scheme||a.word}`).join("\n")
    });
}
